...
class UserModel {
  static async checkEmailNotAdmin(email) {
  ...
  static async register(email, password) {
    ...
    try {
      ...
      if (err.code === '23505') return { error: 'Email already registered as user' };
  ...
  static async registerWithGoogle(email, hashedPassword, googleId) {
    ...
    try {
      ...
      return { error: 'Google registration failed', details: err.message };
    } finally {
      client.release();
    }
  }
