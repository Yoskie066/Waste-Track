...
export default function AdminReport() {
  ...
  const [colorOptions, setColorOptions] = useState(["all"]);
  ...
  useEffect(() => {
    const fetchColors = async () => {
      ...
      try {
        const response = await adminApi.get("/report-waste/colors");
        if (response.data.success) {
          setColorOptions(["all", ...response.data.data]);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      ...
      try {
        const response = await adminApi.get("/report-waste/locations");
        if (response.data.success) {
          setLocationOptions(["all", ...response.data.data]);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchYears = async () => {
      ...
      try {
        const response = await adminApi.get("/report-waste/years");
        if (response.data.success) {
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      const response = await adminApi.get("/report-waste", { params });
      if (response.data.success) {
  ...
  useEffect(() => {
    fetchRecords();
  }, [currentPage, selectedCategory, subCategory, selectedColor, selectedLocation, selectedMonth, selectedYear, sortOrder]);
  ...
  const handleDelete = async () => {
    try {
      const response = await adminApi.delete(`/report-waste/${recordToDelete}`);
      if (response.data.success) {
      ...
      showFeedback("error", error.response?.data?.message || "Error deleting report");
    }
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await adminApi.get("/report-waste/export", {
        params,
        responseType: "blob",
      });
      ...
      link.href = url;
      link.setAttribute("download", "reported_waste_export.xlsx");
      ...
      <div className="w-full max-w-7xl bg-white/70 backdrop-blur-lg rounded-2xl shadow-2xl p-6 md:p-8 border border-white/30">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            View, filter, and manage all waste reports
          </p>
        ...
        {showFilters && (
          <div className="bg-white/50 backdrop-blur-sm border border-gray-200 rounded-xl p-4 mb-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              {loadingColors ? (
                <div className={inputStyle + " text-gray-400"}>Loading colors...</div>
              ) : (
                renderDropdown(colorOptions, selectedColor, setSelectedColor, "Select Color")
              )}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Location</label>
              {loadingLocations ? (
                <div className={inputStyle + " text-gray-400"}>Loading locations...</div>
              ) : (
                renderDropdown(locationOptions, selectedLocation, setSelectedLocation, "Select Location")
              )}
            </div>
            <div>
            ...
            <tbody>
              {records.map((record) => (
                <tr key={record.id} className="border-b border-gray-200 hover:bg-green-50 transition">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">{record.color}</td>
                  <td className="px-4 py-3 truncate max-w-[200px]">{record.location}</td>
                  <td className="px-4 py-3 whitespace-nowrap">{formatDate(record.datereported)}</td>
                ...
                <div className="p-5">
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-800">{record.color}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      <span className="text-gray-500 block text-xs">Date Reported</span>
                      <p className="font-medium text-gray-800">{formatDate(record.datereported)}</p>
                    </div>
                    {record.description && (
        ...
        >
          ...
          >
            {recordToPreview && (
              <>
                <h3 className="text-2xl font-bold text-gray-800 mb-4">Report Details</h3>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
                    ...
                    <p><span className="font-semibold">Color:</span> {recordToPreview.color}</p>
                    <p><span className="font-semibold">Location:</span> {recordToPreview.location}</p>
                    <p><span className="font-semibold">Date Reported:</span> {formatDate(recordToPreview.datereported)}</p>
